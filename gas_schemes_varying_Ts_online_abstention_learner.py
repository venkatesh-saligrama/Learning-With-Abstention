...
def compute_error_bars( Ts, runs, return_stats, label='exp-1-' ):
    ...
    plt.savefig( './gas_plots/' + label + '-mistakes.png' )


    fig = plt.figure()
    plt.errorbar(Xs, Y2, yerr=err_Y2, label='a_t')
    plt.legend(loc='lower right')
    plt.savefig( './gas_plots/' + label + '-abstentions.png' )

    fig = plt.figure()
    plt.errorbar(Xs, Y3, yerr=err_Y3, label='extra_a_t')
    plt.legend(loc='lower right')
    plt.savefig( './gas_plots/' + label + '-mistake-matched-extra-abstentions.png' )
...
if __name__ == '__main__':
    ...
    with open('gas-runs-varying-Ts-400' + str(time.time()) + '.pickle', 'wb') as handle:
    ...
    compute_error_bars( Ts, runs,  return_stats, label='gas-cumulative-Ts-exp--' + str(time.time()) )  
