...
def compute_error_bars( Ts, runs, return_stats, label='exp-1-' ):
    ...
    plt.legend(loc='lower right')
    plt.savefig( base_dir + label + '-mistake-matched-extra-abstentions.png' )

    #discrete
    fig = plt.figure()
    plt.plot(raw_abs, raw_mis, 'o', color='red', label='raw abs vs raw mistakes')
    plt.legend(loc='lower right')
    plt.savefig( base_dir + label + '-comparative_raw_a_vs_m_linear_discrete_varying_Ts.png' )
...
if __name__ == '__main__':
    ...
    with open('gas-runs-varying-Ts-20' + str(time.time()) + '.pickle', 'wb') as handle:
    ...
    compute_error_bars( Ts, runs,  return_stats, label='gas-varying-Ts-exp-P-sqrt(1byT)-' + str(time.time()) )  
