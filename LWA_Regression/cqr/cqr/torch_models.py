...
class LWA_RegressionLoss(nn.Module):
    """ LWA Regression loss function
    """
    def __init__(self, _lambda1=1.0, _lambda2=1.0):
        """ Initialize
    ...
    def forward(self, f, g, target):
        ...
        assert f.size(0) == target.size(0)
        assert g.size(0) == target.size(0)
        y = target
        loss = self.relu(f + g) + self._lambda1 * self.relu(y - f) + + self._lambda2 * self.relu(-y - g) 
        loss = torch.mean(loss)
        return loss



def lwa_epoch_internal_train(model, loss_func, x_train, y_train, batch_size, optimizer, cnt=0, best_cnt=np.Inf):
...
###############################################################################
# Learning with Abstention Regression
# Minimizing custom loss
###############################################################################

# Define the network
class lwa_regression_model(nn.Module):
    ...
    def forward(self, x):
        """ Run forward pass
        """
        return torch.squeeze(self.f(x)), torch.squeeze(self.g(x))



# Define the training procedure
class LWA_LearnerOptimized:
    ...
    def fit(self, x, y, epochs, batch_size, verbose=False):
        ...
        for e in range(epochs):
            epoch_loss, cnt = lwa_epoch_internal_train(model, self.loss_func, x_train, y_train, batch_size, optimizer, cnt)
            ...
            test_preds = ( pred_f.cpu().detach().numpy() + pred_g.cpu().detach().numpy() ) / 2.

            test_preds = np.squeeze(test_preds)
            test_epoch_loss = self.loss_func(pred_f, pred_g, yy).cpu().detach().numpy()
        ...
        for e in range(best_epoch+1):
            ...
            epoch_loss, cnt = lwa_epoch_internal_train(self.model, self.loss_func, x, y, batch_size, self.optimizer, cnt, best_cnt)
